...
namespace RocketryWebApp
{
    public partial class WebForm1 : System.Web.UI.Page
    {
        ...
        {
            resetTextBox(stageNumber, wetMassColumn);
            resetTextBox(stageNumber, dryMassColumn);
            resetTextBox(stageNumber, thrustColumn);
            resetTextBox(stageNumber, ispColumn);
            resetTextBox(stageNumber, minTWRColumn);
            resetTextBox(stageNumber, maxTWRColumn);
            resetTextBox(stageNumber, deltaVColumn);
        }
        private void resetTextBox(int stageNumber, TableCell[] array)
        {
            for (int i = stageNumber; i < 9; i++)
            {
                getTextBox(i, array).Text = getTextBox(i + 1, array).Text;
            }
            getTextBox(9, array).Text = "";
        }
        private TextBox getTextBox(int i, TableCell[] TCArray)
        {
            StringBuilder varID = new StringBuilder(TCArray[i].ID);
            varID.Remove(0, 2);
            varID.Insert(0, "TextBox");
            return (TextBox)TCArray[i].FindControl(varID.ToString());
        }
        #endregion
